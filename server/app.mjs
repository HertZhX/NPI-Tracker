...
const ATTACHMENT_JSON_LIMIT_BYTES = 15 * 1024 * 1024;
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "PATCH" && projectMatch) {
        ...
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.updateProjectDetails(
      ...
      const projectStatusMatch = pathname.match(/^\/api\/admin\/projects\/([^/]+)\/status$/);
      if (request.method === "PATCH" && projectStatusMatch) {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.setProjectStatus(
      ...
      const productMatch = pathname.match(/^\/api\/admin\/projects\/([^/]+)\/products\/([^/]+)$/);
      if (request.method === "PATCH" && productMatch) {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.updateProductDetails(
          decodeURIComponent(productMatch[1]),
          decodeURIComponent(productMatch[2]),
          body,
          body.expectedRevision,
          session.account,
          requestContext(request, session.account.id),
      ...
      if (request.method === "DELETE" && productMatch) {
        ...
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.deleteProduct(
          decodeURIComponent(productMatch[1]),
          decodeURIComponent(productMatch[2]),
          body.expectedRevision,
          session.account,
      ...
      const phaseMatch = pathname.match(/^\/api\/admin\/projects\/([^/]+)\/products\/([^/]+)\/phases\/([^/]+)$/);
      if (request.method === "DELETE" && phaseMatch) {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.deleteStage(
          decodeURIComponent(phaseMatch[1]),
          decodeURIComponent(phaseMatch[2]),
          decodeURIComponent(phaseMatch[3]),
          body.expectedRevision,
          session.account,
          requestContext(request, session.account.id),
        );
      ...
      const phaseTransitionMatch = pathname.match(/^\/api\/admin\/projects\/([^/]+)\/products\/([^/]+)\/phases\/([^/]+)\/transition$/);
      if (request.method === "POST" && phaseTransitionMatch) {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.transitionStage(
          decodeURIComponent(phaseTransitionMatch[1]),
          decodeURIComponent(phaseTransitionMatch[2]),
          decodeURIComponent(phaseTransitionMatch[3]),
          body,
          body.expectedRevision,
          session.account,
          requestContext(request, session.account.id),
      ...
      if (request.method === "POST" && taskAttachmentMatch) {
        session = requireSession(request);
        validateCsrf(request, session);
        const body = await readJson(request, ATTACHMENT_JSON_LIMIT_BYTES);
        const result = repository.syncAttachments(
          "task",
      ...
      if (request.method === "POST" && pathname === "/api/admin/workflow-items") {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.createWorkflowItem(
          body,
          body.expectedRevision,
          session.account,
          requestContext(request, session.account.id),
      ...
      const workflowArchiveMatch = pathname.match(/^\/api\/admin\/workflow-items\/([^/]+)\/archive$/);
      if (request.method === "PATCH" && workflowArchiveMatch) {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.setWorkflowItemArchived(
          decodeURIComponent(workflowArchiveMatch[1]),
          body,
          body.expectedRevision,
          session.account,
          requestContext(request, session.account.id),
      ...
      const workflowAttachmentMatch = pathname.match(/^\/api\/workflow-items\/([^/]+)\/attachments$/);
      if (request.method === "POST" && workflowAttachmentMatch) {
        session = requireSession(request);
        validateCsrf(request, session);
        const body = await readJson(request, ATTACHMENT_JSON_LIMIT_BYTES);
        const result = repository.syncAttachments(
          "workflow_item",
          decodeURIComponent(workflowAttachmentMatch[1]),
          body,
          session.account,
          requestContext(request, session.account.id),
        );
      ...
      if (request.method === "PATCH" && meetingMatch) {
        ...
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.patchMeeting(
          decodeURIComponent(meetingMatch[1]),
          body,
          session.account,
          requestContext(request, session.account.id),
        );
      ...
      const meetingAttachmentMatch = pathname.match(/^\/api\/meetings\/([^/]+)\/attachments$/);
      if (request.method === "POST" && meetingAttachmentMatch) {
        session = requireSession(request);
        validateCsrf(request, session);
        const body = await readJson(request, ATTACHMENT_JSON_LIMIT_BYTES);
        const result = repository.syncAttachments(
          "meeting",
          decodeURIComponent(meetingAttachmentMatch[1]),
          body,
          session.account,
          requestContext(request, session.account.id),
        );
      ...
      if (request.method === "POST" && pathname === "/api/quotations/import") {
        session = requireSession(request);
        validateCsrf(request, session);
        const body = await readJson(request, QUOTATION_JSON_LIMIT_BYTES);
        const quotation = repository.createQuotationImport(
          body,
          session.account,
          requestContext(request, session.account.id),
        );
      ...
      const attachmentFileMatch = pathname.match(/^\/api\/attachments\/([^/]+)\/(preview|download)$/);
      if (request.method === "GET" && attachmentFileMatch) {
        session = requireSession(request);
        const result = repository.getAttachmentFile(
          decodeURIComponent(attachmentFileMatch[1]),
          session.account,
        );
        const fallbackName = result.attachment.name
          .replace(/[^\x20-\x7E]/g, "_")
          .replace(/["\\]/g, "_");
        const disposition = attachmentFileMatch[2] === "preview" ? "inline" : "attachment";
        response.writeHead(200, {
          "Content-Type": result.attachment.type || "application/octet-stream",
          "Content-Length": result.content.length,
          "Content-Disposition": `${disposition}; filename="${fallbackName}"; filename*=UTF-8''${encodeURIComponent(result.attachment.name)}`,
          "Cache-Control": "no-store",
      ...
      if (request.method === "POST" && pathname === "/api/admin/bom-items/bulk-ready") {
        session = requireSession(request);
        requireAdmin(session);
        validateCsrf(request, session);
        const body = await readJson(request);
        const result = repository.bulkReadyBom(
          body,
          body.expectedRevision,
          session.account,
          requestContext(request, session.account.id),
